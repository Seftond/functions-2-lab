...
// CODE HERE
const totalPopulation = populations.reduce((accume, current) => {
  accume = accume + current
  return accume
})
...
orders.forEach(function(item){
  let itemTotal = item["price"] * item["tax"]
  arrayTotals.push(itemTotal)
})
...
// CODE HERE
const bobsTotal = purchases.filter( element => element["owner"] === "Bob").map(element => element["price"]).reduce((accume, current ) => accume + current)
